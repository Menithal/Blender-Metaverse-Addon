...
def fix_vertex_groups(obj):
    ...
    for vertex_group in vertex_groups:  
        if "Waist" in vertex_group.name:
            mix_weights("Hips", "Waist")
            remove_list.append(vertex_group)
        if "Head2" in vertex_group.name:
...
def clean_unused_vertex_groups(obj):
    ...
    for vertex in obj.data.vertices:
        ...
        for group in vertex.groups:
            if group in vertex_groups:
                vertex_groups.remove(group)
                print("  - Removing ", group)
    
    print( " Found ", len(vertex_groups), " without weights, cleaning up")
    print( " Removing Unused Bones")
    _to_remove_bones = []
    if obj.parent.type is "ARMATURE":
        for edit_bone in obj.parent.data.edit_bones:
            if edit_bone.name in vertex_groups:
                print( "  - Removing Bone", edit_bone.name)
                _to_remove_bones.append(edit_bone)

        for bone_to_remove in _to_remove_bones:
            obj.parent.data.edit_bones.remove(bone_to_remove)
