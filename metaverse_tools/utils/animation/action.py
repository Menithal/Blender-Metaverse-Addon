...
class ObjectInfo:
    ...
    def set_info_to_object(self, entity):
        ...
        entity.rotation_quaternion = self.rotation
...
def return_sides(name):
    ...
    raise Exception("Not Mirrorable Name " + name) 
...
def get_bone_frames_in_action(armature, action):
    ...
    for bone in active_bones_actions:
        ...
        for frame in keyframes:
            ...
            info = ObjectInfo(pose_bone.location, pose_bone.rotation_quaternion, pose_bone.scale)
...
def split_bone_frames_into_mirrored_action(armature, action):
    ...
    def check_side (side, short_side, long_side, bone_side): 
        print("check side " + side +" " + short_side  +" "+ long_side  +" "+ bone_side)
        return (side == short_side or side == long_side) and long_side == bone_side
    for side in sides:
        print("Checking Sidess " + side)
        ...
        if action_to_make != None:
            ...
            print("Removing Old Action " + action_name)
        ...
        for bone in keyframed_bones:
            print("Keyframed Bone " + bone.name)
            ...
            if bone.mirrorable != None:
                ...
                if (check_side(side, 'R', 'Right', bone.mirrorable.side) or check_side(side, 'L', 'Left', bone.mirrorable.side)):
                    for frame, info in bone.frame_info_tuple():
                        print(str(frame) + " - " + str(info))
                        create_keyframe(pose_bone, frame, info)
                print("migrate to new pose, specific side only!")
            if bone.mirrorable == None:
                print("not mirrorable , continue as is.")
                for frame, info in bone.frame_info_tuple():
                    print(str(frame) + " - " + str(info))
                    create_keyframe(pose_bone, frame, info)
                print("migrate new pose, without bothering with the side")
